...
int query(vector<int> &segTree,int l,int r,int k,int ind)
{
    if(l==r) return l;
    ...
    if(left>=k)
    {
        return query(segTree,l,mid,k,2*ind+1);
    }
    else if(left+right>=k)
    {
        return query(segTree,mid+1,r,k-left,2*ind+2);
    }
    else return -1;
}
